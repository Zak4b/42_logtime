...
export default class LogTimeExtension extends Extension {
	enable() {
		...
		// 3. Variables pour stocker les données
		...
		try {
			...
			log(`Erreur chargement settings: ${e.message}`);
			this._settings = null;
		}

		// 5. Créer le menu popup
		...
		// 6. Écouter les changements de settings
		if (this._settings) {
			this._settingsChangedId1 = this._settings.connect("changed::display-mode", () => {
				this._updateMenuSwitches();
				this._updateDisplay();
			});
			this._settingsChangedId2 = this._settings.connect("changed::show-departure-time", () => {
				this._updateMenuSwitches();
				this._updateDisplay();
			});
		}
	...
	_updateDisplay() {
		const displayMode = this._settings ? this._settings.get_string("display-mode") : "logtime";
		const showDeparture = this._settings ? this._settings.get_boolean("show-departure-time") : false;
